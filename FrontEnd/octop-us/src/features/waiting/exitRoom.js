  ...
  await axios
    ...
    .then((res) => {
      ...
      if (getRoomInfo.personNum === 1) {
        axios
          .delete(`http://localhost:8080/rooms/${roomId}`)
          .then(() => (document.location.href = "https://localhost:3000/main"));
        // 세션 떠남 알림 필요 없나?
      }
      if (getRoomInfo.roomChief === userName) {
        ...
        axios
          .put("http://localhost:8080/rooms", JSON.stringify(getRoomInfo), {
            headers: {
        ...
        axios
          ...
          .then(() => {
            document.location.href = "https://localhost:3000/main";
          });
